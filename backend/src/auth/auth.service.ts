...
export class AuthService {
    ...
    async googleLogin(googleLoginDto: { idToken: string }): Promise<AuthResponseDto> {
    ...
    async forgotPassword(email: string): Promise<{ message: string }> {
        ...
        if (!user) {
            return {
                message: 'Verification code sent to your email if the address exists in our system.',
            };
        }
    ...
    async googleRegisterStart(
        ...
        const userInstance = this.UserRepository.create({
            ...
            experience_level: googleRegisterStartDto.experience_level as ExperienceLevel,
            is_google_user: true,
    ...
    async googleRegisterVerify(
        ...
        if (!user || user.google_verification_code !== googleRegisterVerifyDto.code) {

...
describe("http client", () => {
  ...
  it("returns null from refresh when no refresh token is available", async () => {
  ...
  it("refreshes without saving user when response has no user object", async () => {
    const mockInstance = createMockInstance();
    const { responseErrorHandler } = await loadHttpModuleWith(mockInstance);
    mockAxiosPost.mockResolvedValue({
      data: {
    ...
    mockInstance.instance.mockResolvedValue({ data: { ok: true } });
    ...
    expect(mockSaveAuthTokens).toHaveBeenCalled();
  });
  ...
  it("returns plain string message from toApiError when response message is a string", async () => {
    const mockInstance = createMockInstance();
    const { toApiError } = await loadHttpModuleWith(mockInstance);

    const error = {
      message: "Request failed",
      response: { status: 400, data: { message: "Bad request details" } },
    };

    expect(toApiError(error)).toEqual({
      status: 400,
      message: "Bad request details",
      details: { message: "Bad request details" },
    });
  });

  it("falls back to axios message when response data message is empty string", async () => {
    const mockInstance = createMockInstance();
    const { toApiError } = await loadHttpModuleWith(mockInstance);
  ...
  it("does not add authorization when access token is missing", async () => {
  ...
  it("skips 401 retry when no auth session exists", async () => {
    mockGetAccessToken.mockReturnValue(null);
    mockGetRefreshToken.mockReturnValue(null);
    const mockInstance = createMockInstance();
    const { responseErrorHandler } = await loadHttpModuleWith(mockInstance);
  ...
  it("clears session when refresh returns null token", async () => {
    const mockInstance = createMockInstance();
    const { responseErrorHandler } = await loadHttpModuleWith(mockInstance);
    mockAxiosPost.mockResolvedValue({
  ...
  it("handles error config without headers object", async () => {
    const mockInstance = createMockInstance();
    const { responseErrorHandler } = await loadHttpModuleWith(mockInstance);
    mockAxiosPost.mockResolvedValue({
      data: {
    ...
    mockInstance.instance.mockResolvedValue({ data: {} });
  ...
  it("handles error without config object", async () => {
    const mockInstance = createMockInstance();
    const { responseErrorHandler } = await loadHttpModuleWith(mockInstance);
  ...
  it("uses fallback message when axiosError.message is undefined", async () => {
    const mockInstance = createMockInstance();
    const { toApiError } = await loadHttpModuleWith(mockInstance);
